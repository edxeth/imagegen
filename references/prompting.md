...
These prompting principles apply to the Pi image-generation CLI in `scripts/image_gen.py`.

This file is about prompt structure, specificity, and iteration. Execution controls such as `quality`, `input_fidelity`, masks, output format, endpoints, and output paths live in `references/cli.md` and `references/image-api.md`.
...
## Execution controls
- `quality`, `input_fidelity`, explicit masks, output format, endpoints, and output paths are CLI execution controls.
- Keep visual intent in the prompt and mechanical controls in CLI flags.
